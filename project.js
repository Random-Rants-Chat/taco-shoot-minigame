...
class CloudProvider {
    ...
    onClose (e) {
        ...
        if (e && e.code === 4004) {
            console.warn('Cloud variables are disabled for this project. Not reconnecting.');
            return;
        }
    ...
    setTimeout (fn, time) {
        console.warn(`Reconnecting in ${(time / 1000).toFixed(1)}s, attempt ${this.connectionAttempts}`);
...
var provider = new CloudProvider(
	urlParams.get("cloud_host")||websocketServerURL,
	vm,
	options.username,
	`${projectID}`
);
